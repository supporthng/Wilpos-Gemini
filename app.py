...
def run_visual_countdown(total_seconds=20, text_msg="Límite temporal alcanzado (15 peticiones por minuto). Pausa de seguridad activa:"):
    ...
    for intento in range(2):
        try:
            ...
            if "429" in err_str or "quota" in err_str.lower() or "RESOURCE_EXHAUSTED" in err_str:
                if intento < 1:
                    run_visual_countdown(20, "Límite de peticiones alcanzado. Esperando reintento automático:")
                    continue
...
if modulo == "📄 Factura Individual":
    ...
    st.markdown("<p style='color: #64748b;'>Procesamiento seguro con Gemini Flash.</p>", unsafe_allow_html=True)
    st.markdown("---")
...
# ==========================================
    ...
    st.markdown("<p style='color: #64748b;'>Procesa múltiples facturas con manejo de reintentos.</p>", unsafe_allow_html=True)
    st.markdown("---")
        ...
        if st.session_state.get("is_live_processing", False):
            if processed_so_far < total_files:
                ...
                if err_msg == "QUOTA_EXCEEDED":
                    run_visual_countdown(20, "Límite temporal alcanzado en lote. Conteo regresivo visual para reanudar:")
                    st.rerun()
