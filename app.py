...
# ==========================================
st.set_page_config(
    page_title="WilPOS - Prioridad Códigos EAN Estándar", 
    page_icon="⚡", 
    layout="wide",
...
def render_master_status_banner():
    ...
    st.success(f"🟢 **WilPOS Activo** | Catálogo Maestro: **{len(master_dict):,}** productos | 🕒 Última act: **{meta.get('ultima_actualizacion', 'Desconocida')}**")
...
def get_strict_ean_code_and_name(description, invoice_ean=""):
    ...
    # 1. Prioridad absoluta: Si está en el Catálogo Maestro oficial con su EAN estándar, usamos ese.
    if desc_clean in master:
        return desc_clean, clean_ean_code(master[desc_clean])
        
    # 2. Si está en la memoria de aprendizaje previa
    if desc_clean in memory:
        return desc_clean, clean_ean_code(memory[desc_clean])
        
    # 3. Si la factura trajo un código EAN válido de 13 dígitos directo
    cleaned_invoice_ean = clean_ean_code(invoice_ean)
    if cleaned_invoice_ean != "S/C (Sin Código)":
        return desc_clean, cleaned_invoice_ean
        
    # 4. Coincidencia difusa contra el Catálogo Maestro
    ...
    if master_keys:
        ...
        if coincidencias:
            ...
            return desc_clean, clean_ean_code(master[matched_name])
            
    return desc_clean, cleaned_invoice_ean

def parse_empaque_exact(unidad_txt, descripcion_txt=""):
    ...
    match_caj = re.search(r'CAJ[A-]?\s*/?\s*(\d+)', combined)
    if match_caj: return int(match_caj.group(1))
...
st.sidebar.markdown("<p style='text-align: center; color: #64748b; font-size: 0.8rem;'>Prioridad Absoluta Códigos EAN</p>", unsafe_allow_html=True)
st.sidebar.markdown("---")

modulo = st.sidebar.radio("Menú de Navegación", ["📄 Factura Individual", "📂 Múltiples Facturas (Lote)", "📁 Actualizar Catálogo Maestro", "📋 Códigos Almacenados"])

st.sidebar.markdown("---")
use_gemini_paid_api = st.sidebar.checkbox("💎 Usar Gemini Paid (API de Pago)", value=bool(ACTIVE_GEMINI_PAID_KEY))

def process_invoice_strict_ean(file_obj, file_type, use_paid_gemini=False):
    prompt_text = (
        "Analiza esta factura con máxima precisión. "
        "REGLA DE ORO DE CÓDIGOS DE BARRAS: Debes buscar y extraer estrictamente el **código de barras EAN estándar de 13 dígitos** impreso en la línea (ignora por completo códigos internos SAP o numéricos cortos si hay un EAN presente). "
        "Para cada renglón extrae rigurosamente en un JSON bajo la clave 'items': "
        "1. 'codigo_ean': el código de barras EAN estándar de 13 dígitos (ej: 7804300120986). "
        "2. 'descripcion': nombre exacto y literal del producto en la factura. "
        "3. 'cantidad': cantidad de cajas compradas. "
        "4. 'unidad': unidad de medida o presentación (ej: 'CAJ / 12 PZA', 'Caja-24'). "
        "5. 'valor': monto total neto de la línea. "
        "Estructura JSON exacta: "
        '{"items": [{"codigo_ean": "7804300120986", "descripcion": "CERVEZA CORONA CERO 355ML", "cantidad": 1.0, "unidad": "Caja-24", "valor": 2024.88}]}. '
...
if modulo == "📄 Factura Individual":
    try:
        st.markdown("<h2>📊 Automatizador de Facturas <span style='color: #0284c7;'>(Prioridad EAN Estándar)</span></h2>", unsafe_allow_html=True)
        st.markdown("<p style='color: #64748b;'>Extracción forzada de códigos de barras EAN y cálculo exacto de costos.</p>", unsafe_allow_html=True)
        st.markdown("---")
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 Procesar con Códigos EAN Prioritarios"):
                file_type = getattr(uploaded_file, 'type', 'image/jpeg')
                with st.spinner("Procesando documento y priorizando códigos EAN..."):
                    parsed_data, success_msg = process_invoice_strict_ean(uploaded_file, file_type, use_paid_gemini=use_gemini_paid_api)

                if success_msg == "QUOTA_EXCEEDED":
                    st.error("⚠️ Límite de cuota alcanzado. Activa 'Gemini Paid' en la barra lateral.")
                elif not parsed_data or not isinstance(parsed_data, dict):
                    st.error(f"⚠️ Error al procesar: {success_msg}")
                else:
                    st.success("¡Factura procesada con prioridad EAN!")
                    ...
                    for idx, item in enumerate(data_items, start=1):
                        ...
                        empaque_val = parse_empaque_exact(unidad_txt, resolved_name)
                        ...
                        rows_preview.append({
                            "No.": idx,
                            "Código EAN Estándar": str(resolved_code),
                    ...
                    if rows_preview:
                        st.markdown("### ✅ Artículos con Códigos EAN Estándar Prioritarios")
                        df_resultado = pd.DataFrame(rows_preview)
                        df_resultado["Código EAN Estándar"] = df_resultado["Código EAN Estándar"].astype(str)
                        ...
                        for item_dict in rows_preview:
                            ws_prod.append([
                                item_dict["Nombre Producto"], str(item_dict["Código EAN Estándar"]),
                        ...
                        output = io.BytesIO()
                        wb.save(output)
                        st.download_button("📥 Descargar Excel WilPOS EAN Oficial", output.getvalue(), "Inventario_WilPOS_EAN.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    except Exception as e:
...
# ==========================================
    ...
    try:
        st.markdown("<h2>📂 Procesador por Lotes <span style='color: #0284c7;'>(Prioridad EAN)</span></h2>", unsafe_allow_html=True)
        st.markdown("---")
        render_master_status_banner()
        st.markdown('<div class="card-container">', unsafe_allow_html=True)
        ...
        if "cached_uploaded_files" in st.session_state and st.session_state["cached_uploaded_files"]:
            ...
            if b_col1.button("🚀 Iniciar Lote con EAN", type="primary"):
            ...
            if st.session_state.get("is_live_processing", False):
                if processed_so_far < total_files:
                    ...
                    parsed_data, _ = process_invoice_strict_ean(io.BytesIO(file_info["bytes"]), file_info["type"], use_paid_gemini=use_gemini_paid_api)
                    time.sleep(1.0)
