...
def process_invoice_with_ai(file_obj, file_type, use_openai_fallback=False):
    if use_openai_fallback:
        ...
        prompt_text = (
            ...
            "Para cada ítem, extrae estrictamente: 'descripcion', 'cantidad', 'empaque', 'precio_lista' (precio unitario o caja antes de descuento), "
            "y 'descuento_porcentaje' (ej. 10 si tiene 10%, o 0). "
...
# ==========================================
    ...
    if "cached_uploaded_files" in st.session_state and st.session_state["cached_uploaded_files"]:
        ...
        if st.session_state.get("quota_paused", False):
            st.error("⚠️ **Límite de cuota gratuita alcanzado (Error 429).**")
