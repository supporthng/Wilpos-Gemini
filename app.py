...
# Memoria de Proveedores Activa y Persistente
if "supplier_memory" not in st.session_state:
    loaded_suppliers = load_json_file(SUPPLIER_MEMORY_FILE)
    if not loaded_suppliers:
        loaded_suppliers = {
            "PRICESMART": {"nombre": "PRICESMART", "formato": "tiquet_pos", "regla": "Códigos numéricos de ítem y descripciones directas."},
            "GONZALEZ CUESTA": {"nombre": "GONZALEZ CUESTA", "formato": "credito_fiscal", "regla": "Doble código (SAP y EAN de 13 dígitos) en columna izquierda."}
        }
...
def get_resolved_code_and_name(description, invoice_code=""):
    ...
    if desc_clean in master: return desc_clean, clean_code(master[desc_clean])
    if desc_clean in memory: return desc_clean, clean_code(memory[desc_clean])
    master_keys = list(master.keys())
    if master_keys:
        coincidencias = difflib.get_close_matches(desc_clean, master_keys, n=1, cutoff=0.80)
        if coincidencias: return coincidencias[0], clean_code(master[coincidencias[0]])
    return desc_clean, "S/C (Sin Código)"

def parse_empaque_exact(unidad_txt, descripcion_txt="", supplier_name=""):
    u = str(unidad_txt).strip().upper()
    d = str(descripcion_txt).strip().upper()
    combined = d + " " + u
    
    if any(k in supplier_name for k in ["GONZALEZ", "CUESTA"]):
    ...
    match_pza = re.search(r'(\d+)\s*PZA', combined)
    if match_pza: return int(match_pza.group(1))
    match_dash = re.search(r'CAJA\s*-\s*(\d+)', combined)
    if match_dash: return int(match_dash.group(1))
...
st.sidebar.markdown("<p style='text-align: center; color: #64748b; font-size: 0.8rem;'>Con Memoria de Proveedores Activa</p>", unsafe_allow_html=True)
st.sidebar.markdown("---")
...
def process_invoice_with_memory(file_obj, file_type, use_paid_gemini=False):
    # Detección inteligente del proveedor
    prompt_detect = "Identifica el nombre comercial del proveedor de esta factura. Devuelve un JSON puro: {'proveedor': 'NOMBRE'}"
    ...
    # Registrar en memoria si es nuevo
    supp_mem = st.session_state["supplier_memory"]
    if supplier_detected not in supp_mem and supplier_detected != "GENERAL":
        supp_mem[supplier_detected] = {"nombre": supplier_detected, "formato": "aprendido_automatico"}
        save_supplier_memory()

    prompt_main = (
        f"Analiza esta factura del proveedor '{supplier_detected}' con absoluta precisión matemática. "
        "Extrae rigurosamente cada renglón en un JSON bajo la clave 'items': "
        "1. 'codigo_articulo': ID o código del producto. "
        "2. 'descripcion': nombre exacto del producto. "
        "3. 'cantidad': cantidad de cajas o bultos comprados. "
        "4. 'unidad': texto de la unidad o empaque (ej: 'Caja-24', 'CAJ / 12 PZA'). "
        "5. 'valor': monto total neto de la línea. "
        "Estructura JSON exacta: "
        '{"proveedor": "' + supplier_detected + '", "items": [{"codigo_articulo": "...", "descripcion": "...", "cantidad": 1.0, "unidad": "...", "valor": 0.0}]}. '
...
if modulo == "📄 Factura Individual":
    try:
        ...
        st.markdown("<p style='color: #64748b;'>Detección de proveedor, costos unitarios reales y empaques correctos.</p>", unsafe_allow_html=True)
        st.markdown("---")
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 Procesar con Memoria de Proveedor"):
                file_type = getattr(uploaded_file, 'type', 'image/jpeg')
                with st.spinner("Procesando documento..."):
                    parsed_data, success_msg = process_invoice_with_memory(uploaded_file, file_type, use_paid_gemini=use_gemini_paid_api)

                if success_msg == "QUOTA_EXCEEDED":
                    st.error("⚠️ Límite de cuota alcanzado. Activa la casilla de 'Gemini Paid'.")
                    ...
                    for idx, item in enumerate(data_items, start=1):
                        ...
                        invoice_code = str(item.get("codigo_articulo") or "")
                        ...
                        empaque_val = parse_empaque_exact(unidad_txt, resolved_name, prov_det)
                        total_unidades = int(cant_comprada * empaque_val)

                        # Costo unitario real corregido
                        ...
                        rows_preview.append({
                            "No.": idx,
                            "Código Artículo": str(resolved_code),
                    ...
                    if rows_preview:
                        ...
                        df_resultado["Código Artículo"] = df_resultado["Código Artículo"].astype(str)
                        ...
                        for item_dict in rows_preview:
                            ws_prod.append([
                                item_dict["Nombre Producto"], str(item_dict["Código Artículo"]),
...
# ==========================================
    ...
    try:
        st.markdown("<h2>📂 Procesador por <span style='color: #0284c7;'>Lotes con Memoria</span></h2>", unsafe_allow_html=True)
        st.markdown("---")
        render_master_status_banner()
        st.markdown('<div class="card-container">', unsafe_allow_html=True)
        ...
        if "cached_uploaded_files" in st.session_state and st.session_state["cached_uploaded_files"]:
            ...
            if b_col1.button("🚀 Iniciar Lote", type="primary"):
            ...
            if st.session_state.get("is_live_processing", False):
                if processed_so_far < total_files:
                    ...
                    parsed_data, _ = process_invoice_with_memory(io.BytesIO(file_info["bytes"]), file_info["type"], use_paid_gemini=use_gemini_paid_api)
                    time.sleep(1.0)
...
# ==========================================
    ...
    try:
        ...
        if master_file is not None:
            ...
            col_code = st.selectbox("Columna con Código", cols)
...
# ==========================================
    ...
    try:
        ...
        st.markdown("<p style='color: #64748b;'>Proveedores detectados y aprendidos por el sistema.</p>", unsafe_allow_html=True)
        st.markdown("---")
        supps = st.session_state["supplier_memory"]
        if supps:
            for s_name, s_data in supps.items():
                with st.expander(f"🏢 {s_name}"):
                    st.write(f"**Formato registrado:** {s_data.get('formato', 'N/A')}")
        else:
...
# ==========================================
    ...
    try:
        st.markdown("<h2>📋 Memoria de Códigos</h2>", unsafe_allow_html=True)
        render_master_status_banner()
        st.markdown("---")
        master = st.session_state["master_catalog"]
        if master:
            st.dataframe(pd.DataFrame([{"Producto": k, "Código": v} for k, v in master.items()]), use_container_width=True, hide_index=True)
        else:
